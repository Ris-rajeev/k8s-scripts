...
MASTER_PRIVATE_IP=$(cat master_private_ip.txt)  # Assuming the IP is saved in master_private_ip.txt
echo $MASTER_PRIVATE_IP

chmod 400 k8s-key-pair.pem

# Step 2: SSH into the master node and copy kubeadm_join.sh to the worker
scp -i k8s-key-pair.pem -o StrictHostKeyChecking=no  rajeev@$MASTER_PRIVATE_IP:/home/rajeev/kubeadm_join.sh kubeadm_join.sh
